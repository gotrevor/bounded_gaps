...
`[0,1]`. The single analytic ingredient of the weighted Mertens asymptotic. Proved by Aristotle
(`930e468a`, `aristotle-wmertens/`); verified `#print axioms` clean in our v4.29.1 kernel
(`[propext, Classical.choice, Quot.sound]`, no `sorry`) and ported here under
`BoundedGaps.WeightedMertens.Riemann`. The previously-disclosed `axiom` in `WeightedMertens.lean`
is now discharged — `weighted_mertens` is fully axiom-clean.
-/
import Mathlib
...
namespace BoundedGaps.WeightedMertens.Riemann
...
end BoundedGaps.WeightedMertens.Riemann