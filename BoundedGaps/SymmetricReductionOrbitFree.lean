...
/-- **Denominator bridge — the combinatorial core (NOW A KERNEL-PROVEN THEOREM, was an axiom).**
The orbit-sum of `∏ᵢ(pᵢ+qᵢ)!` over the two permutation orbits equals the matchings sum
`matchDenSum`, up to the `autParts` over-count from repeated parts (the permanent/rook expansion
grouped by overlap pattern). Discharged via the permanent route: `lhs_eq_perm` rewrites the
orbit double-sum as `k!·permDenSum` (orbit-stabilizer), `rhs_eq_perm` rewrites `matchDenSum` as
`k!·permDenSum` (induction on `La` using `matchDenSum_cons_eq'` / `permDenSum_laplace`), and the
shared `permDenSum` is cancelled. Kernel-clean `[propext, Classical.choice, Quot.sound]`. -/
...
/-- Every matching records at most `|La|` pairs (via the `matchDataN`→`matchData` projection). -/
...
/-- **`matchNumSum` recursion** (numerator analog of `matchDenSum_cons_eq'`), NOW A KERNEL-PROVEN
THEOREM (was a disclosed axiom). The new first row's `gWeight a (·)` pulls out a `matchDenSum` term,
so the recursion couples `matchNumSum` and `matchDenSum` at `k-1`. Proof: split `matchDataN (a::La)`
into the unmatched (`pp=0`) and matched (`pp=1`, over the `Lb`-columns) branches; the shared
per-entry identity is `weight_elt` (front-factor `descFactorial_succ_eq'`); the `gWeight`/`matchNumSum`
half folds back by `rfl`, the factorial-product/`matchDenSum` half by `matchDenSum_dataN`. -/
